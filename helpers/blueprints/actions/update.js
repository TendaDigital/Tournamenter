 ...
 *
 */
module.exports = function updateOneRecord (req, res, Model) {
  ...
  //   console.warn('Cannot change primary key via update blueprint; ignoring value sent for `' + Model.primaryKey + '`');
  // }
  ...
  query.exec(function found(err, matchingRecord) {
    ...
    if (!matchingRecord) return res.status(404).send('No object found with the specified `id`');
    ...
    Model.update(pk, values).exec(function updated(err, records) {
      ...
      Q.exec(function foundAgain(err, populatedRecord) {
        if (err) return res.status(500).send(err);
        if (!populatedRecord) return res.status(500).send('Could not find record after updating!');
        res.send(populatedRecord);
      }); // </foundAgain>
    });// </updated>
  }); // </found>
};
